...
public class roomReset : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject box1, box2, box3, box4, box5,
    ...
    void Update()
    {
        ...
        // Tracking
        Vector2 thisPos = new Vector2(playerTracker.transform.position.x,
                                      playerTracker.transform.position.z);
        // (-10, -10) is the default / reset vector
        if (!prevPos.Equals(new Vector2(-10.0f, -10.0f)))
        {
            float thisDistance = Vector2.Distance(thisPos, prevPos);
            roundDistance += thisDistance;
        }
        ...
        if (score >= 5)
        {
            ...
            if (waiting)
            {
                if (roundTimer >= waitLimit)
                {
                    ...
                    /* Debug.Log("Rounds: " + rounds + ", Round Time: " + roundTimer +
                              ", Round Distance" + roundDistance); */
                    // WIP: export
                    // Export to excel: time, distance, errors
    ...
    void Reset()
    {
        Debug.Log("Distance: " + roundDistance);
        // Per round values
        rounds++;
        roundErrors = 0;
        roundTimer = 0f;
    ...
    // Sets which Boxes are marked based on degree of rotation
